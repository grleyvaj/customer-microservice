...
public class CustomerCreatedEventListener {
	...
	public void handleCustomerCreated(CustomerCreatedDomainEvent event) {
		log.info("AFTER_COMMIT - publishing customer created event for clientId={}", event.getClientId());

		try {
			this.retryTemplate.execute(
			  context -> {
				  log.info(
					"AFTER_COMMIT - publishing customer created event for clientId={}, attempt={}",
					event.getClientId(),
					context.getRetryCount() + 1
				  );
				  this.publisherService.publishCustomerCreated(event.getCustomer());
				  return null;
			  }, recoveryContext -> {
				  // Se agotaron los reintentos: lógica de fallback
				  Throwable lastThrowable = recoveryContext.getLastThrowable();
				  log.error(
					"Exhausted retries publishing customer created event for clientId={} - last error: {}",
				  ...
				  // outboxRepository.save( OutboxEvent.ofCustomerCreated(event.getCustomer(), lastThrowable) );
				  return null;
			  }
			);
		} catch(Exception ex) {
