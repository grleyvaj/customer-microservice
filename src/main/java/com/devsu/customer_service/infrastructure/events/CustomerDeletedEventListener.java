...
public class CustomerDeletedEventListener {
	...
	@Async
	public void handleCustomerDeleted(CustomerDeletedDomainEvent event) {
		log.info("AFTER_COMMIT - publishing customer deleted event for clientId={}", event.getClientId());

		try {
			this.retryTemplate.execute(
			  context -> {
				  log.info(
					"AFTER_COMMIT - publishing customer deleted event for clientId={}, attempt={}",
					event.getClientId(),
					context.getRetryCount() + 1
				  );
				  this.publisherService.publishCustomerDeleted(event.getCustomer());
				  return null;
			  }, recoveryContext -> {
				  // Se agotaron los reintentos: lógica de fallback
				  Throwable lastThrowable = recoveryContext.getLastThrowable();
				  log.error(
					"Exhausted retries publishing customer deleted event for clientId={} - last error: {}",
				  ...
				  // outboxRepository.save( OutboxEvent.ofCustomerDeleted(event.getCustomer(), lastThrowable) );
				  return null;
			  }
			);
		} catch(Exception ex) {
